...
const Modal = ({open, onClose, children, update}) => {
  return (
    // Overlay
    <div onClick={onClose} className={`fixed inset-0 flex justify-center items-center transition-colors ${open ? "visible bg-black/20" : "invisible"} `} >
        ...
        {/* modal*/}
        <div onClick={(e) => e.stopPropagation()} className={`bg-white ${update ? "w-2/5" : "w-1/4"} rounded-xl shadow p-6 transition-all ${open ? "scale-100 opacity-100" : "scale-125 opacity-0"}`}>
